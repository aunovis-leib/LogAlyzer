...
`LogAlyzer` parser tooling and exposes the already parsed log lines.

### `list_parser_profiles`

- Input: `{}`
- Output: the parser profiles configured in the LogAlyzer tool
...
- Input:
  ...
  - `pipeName` (string, optional) – default `LogAlyzer.LiveTools`
- Output:
  - Snapshot of currently loaded parsed entries from the **running LogAlyzer app**.
...
```powershell
dotnet run --project LogAlyzer.McpServer/LogAlyzer.McpServer.csproj
```
...
For all `get_live_*` and `select_live_entry` tools, the desktop app `LogAlyzer` must be running.
The app hosts a local named pipe endpoint (`LogAlyzer.LiveTools`) and returns
...
- args:
  - `run`
  - `--project`
  - `D:/Git/Projekte/LogAlyzer/LogAlyzer.McpServer/LogAlyzer.McpServer.csproj`

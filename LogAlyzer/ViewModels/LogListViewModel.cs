...
public partial class LogListViewModel : ObservableObject, INotifyDataErrorInfo
{
    ...
    private async Task ChooseFile()
    {
        ...
        // Setze Explorer auf das Verzeichnis der ersten gew�hlten Datei
            ...
            try
            {
                ...
                async Task LoadEntriesWithParserAsync(ILogParser effectiveParser)
                {
                    ...
                    await foreach (var chunk in loader.LoadAsync(fileNames, 2000, token))
                    {
                        ...
                        LoadingStatus = $"Geladen: {loadedEntries:N0} Eintr�ge";
    ...
    partial void OnFilterFromTimeTextChanged(string value)
    {
        ...
        SetErrors(nameof(FilterFromTimeText), ["Ung�ltige Uhrzeit. Bitte HH:mm:ss verwenden."]);
    }
    ...
    partial void OnFilterToTimeTextChanged(string value)
    {
        ...
        SetErrors(nameof(FilterToTimeText), ["Ung�ltige Uhrzeit. Bitte HH:mm:ss verwenden."]);
    }
    ...
    /// Dies wird automatisch aufgerufen, wenn ein neuer Eintrag hinzugef�gt wird.
    /// </summary>
    private void ApplyPatternsToEntry(LogFileEntry entry)
    {
        ...
        // Pattern-Matching wird ausschlie�lich zur Speisung des Pattern-Match-Panels
        // ben�tigt. Ist das Panel deaktiviert, entf�llt die (teure) Auswertung pro Eintrag.
        ...
        try
        {
            ...
            if (matches.Any())
            {
                _logger.LogDebug("Patterns f�r einen Logeintrag gefunden");

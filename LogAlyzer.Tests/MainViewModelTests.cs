...
namespace LogAlyzer.Tests
{
    ...
    public class MainViewModelTests
    {
        ...
        private static async Task WaitForSearchResultsAsync(MainViewModel vm, int expectedCount)
        {
            var timeout = DateTime.UtcNow.AddSeconds(2);
            while (vm.SearchResults.Count != expectedCount && DateTime.UtcNow < timeout)
            {
                await Task.Delay(25).ConfigureAwait(false);
            }

            Assert.Equal(expectedCount, vm.SearchResults.Count);
        }
        ...
        public void Search_Finds_Matches_In_Text_RawLine_And_Detail_Across_Lists()
        {
            StaTestHelper.Run(async () =>
            {
                ...
                await WaitForSearchResultsAsync(vm, 3);
        ...
        public void Selecting_Search_Result_Synchronizes_Global_SelectedEntry()
        {
            StaTestHelper.Run(async () =>
            {
                ...
                vm.GlobalSearchText = "needle";
                await WaitForSearchResultsAsync(vm, 1);
